...
console.log(
  "Ont ils tous été emprunté au moins une fois V/F?",
  allrented(books)
);

const mostrented = (array) => {
  return array.reduce((maxRentedBook, currentBook) => {
    return currentBook.rented > maxRentedBook.rented
      ? currentBook
      : maxRentedBook;
  });
};
...
const leastRented = (array) => {
  return array.reduce((leastRentedBook, currentBook) => {
    return currentBook.rented < leastRentedBook.rented
      ? currentBook
      : leastRentedBook;
  });
};
...
console.log(
  "tous les livres sauf celui avec un specific Id :(133712)",
  filterById(books, 133712)
);
